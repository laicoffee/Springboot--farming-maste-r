...
public class UserController {
    ...
    public String logincheck(HttpServletRequest request, HttpSession session, Model m) throws IOException {
        ...
        if (user == null) {
            ...
            System.out.println("这是管理员登录咯，usercontroller方法中");
            List<User> allUser = userService.getAllUser();
            int count = 0;
            for (User u:allUser
                 ) {
                if(u.getName().equals("admin")) {
                    allUser.remove(count);
                    break;
                }
                count++;
            }
            m.addAttribute("users", allUser);
            return "farmer/editor";
        } else {
    ...
    public String deleteUser(@PathVariable("userid") Integer userid, Model m) {
        System.out.println("正在删除,要删除的用户id为" + userid);
        // 在这里编写删除逻辑
